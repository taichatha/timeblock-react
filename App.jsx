...
App = React.createClass({
  ...
  renderHours(){
    ...
    for(var j = 0; j<Tasks.find().fetch().length; j++){
      taskStartTime = Tasks.find().fetch()[j].startTime;
      taskEndTime = Tasks.find().fetch()[j].endTime;
      taskName = Tasks.find().fetch()[j].text;

      for (var i = taskStartTime; i < taskEndTime; i++) {
          indents[i]=
          <tr className={i}>
          <td>{i}</td>
          <td className="hour-cell" id={taskName}>{taskName}</td>
        </tr>
  ...
  addTasksToSchedule(){
    ...
    for(var j = 0; j<Tasks.find().fetch().length; j++){
      ...
      for (var i = taskStartTime; i < taskEndTime; i++) {
          document.getElementById(i).innerHTML = taskName;
